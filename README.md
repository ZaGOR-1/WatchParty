...
- Синхронізація у реальному часі подій:
  ...
  - `playlistUpdated`
  - `playlistAdvanced`
  - `setControlMode` (перемикання `all` / `host`)
  - `syncRequest`
  - `syncResponse`
...
- Режим керування кімнатою:
  - `all` (керувати можуть усі)
  - `host` (керує тільки хост)
  - якщо хост виходить, роль автоматично переходить до наступного учасника
...
```json
{
  ...
  "controlMode": "host",
  "hostSocketId": "Pj8xY3P9Dnz2X4dAAAAB",
  "hostNickname": "Zagor",
  "isPlaying": false,
  "currentTime": 0,
  "stateCurrentTime": 0,
...
7. Перевірте режим `керує тільки хост`:
   - у вкладці хоста натисніть “Увімкнути режим хоста”.
   - у вкладці іншого учасника спробуйте `Play/Pause/Seek` або змінити плейлист.
   - дії мають бути заблоковані локально, а стан автоматично повертається до server snapshot.

## Відомі обмеження MVP

- Немає повноцінної авторизації/акаунтів:
  - режим `host` прив’язаний до поточної Socket.IO-сесії.
  - після повного розриву сесії роль хоста переходить до наступного учасника.
...
- Гранулярні ролі (`host`, `co-host`, `viewer`) з правами на окремі дії.
